...
每个样本的信号数量反映了对应电路模块的端口复杂度。数据集中样本的信号数量分布如表~\ref{tab:signal-dist}所示。

\begin{table}[htbp]
  \centering
  \caption{样本信号数量分布}
  \label{tab:signal-dist}
  \begin{tabular}{crr}
    \toprule
    信号数 & 样本数 & 占比 \\
    \midrule
...
信号数量主要集中在5至10个之间，占比约67\%，中位数为8个信号。这一分布覆盖了从简单门电路（3--4个端口）到中等复杂度模块（13--15个端口）的广泛范围，为模型训练提供了不同复杂度层次的样本。

\subsection{Verilog代码长度分布}

Verilog源代码的长度反映了模块的逻辑复杂度，同时也决定了第二阶段模型生成任务的难度。本文在去除注释与空行后，分别统计了各样本的有效代码行数与词元（Token）数，分布如图~\ref{fig:code-length}所示。
...
代码行数的中位数为61行，均值为89行，主要集中在21至120行之间（约75\%）。词元数的中位数为350，均值为536，主要集中在101至700之间（约78\%）。两个维度均呈现右偏分布，少数大型模块（如总线协议控制器、多级流水线运算单元）的代码长度显著高于平均水平。整体而言，数据集中的模块以中等规模为主，兼顾了训练效率与逻辑复杂度的多样性。
...
为评估微调的效果，本文将微调模型与多个通用多模态大语言模型进行对比，使用相同的测试集与评估流程。基线模型包括Qwen3-VL系列的大参数量版本，均以零样本方式直接推理。表~\ref{tab:stage1-comparison}汇总了各模型的对比结果。
...
通用多模态模型在零样本设置下的表现显著低于微调模型。参数量最大的Qwen3-VL-235B-A22B-Thinking通过启用思维链推理获得了基线中最高的平均F1（0.334），但仍远低于本文4B参数的微调模型（0.969）。解析成功率方面，通用模型的输出格式遵从性较低，仅64\%至88\%的样本能被成功解析为合法WaveJSON，而微调模型达到99.2\%。这一对比表明，时序图的结构化解析是一项高度专业化的任务，即使具备强大视觉理解能力的大规模通用模型也无法通过零样本推理有效完成，领域特定的微调不可或缺。

在117个测试样本中，116个样本（99.2\%）的模型输出被成功解析为合法的WaveJSON结构，仅1个样本因输出格式异常导致解析失败，F1计为零。各样本的F1分数分布如图~\ref{fig:stage1-f1}所示。
...
仅考虑成功解析的116个样本，平均F1达到0.977，中位数为1.000。73个样本（62.9\%）的F1为1.0，表明模型在这些样本上完整且准确地还原了时序图中所有信号的名称、波形序列与数据标签。F1低于0.95的样本仅有14个（12.1\%），F1低于0.8的样本仅有4个（3.4\%），说明模型在绝大多数样本上的解析质量较高。

\subsection{错误模式分析}

对F1低于0.95的14个样本进行信号级对比分析，可归纳出以下两类主要错误模式。

\textbf{波形长度偏差。}最常见的错误类型是模型预测的波形字符串长度与标准答案不一致，即模型对时序图中的时钟周期总数估计有误。在14个低分样本中，超过半数存在此类问题。典型表现为模型将20个周期的波形扩展为40个周期（约为两倍）。这一错误源于时序图图像中时钟周期的视觉分辨率限制——当波形密集、周期间距较小时，模型可能将两个连续周期误认为一个，或将一个周期的上升沿与下降沿分别计为两个独立周期。波形长度的偏差会导致信号的逐周期对齐产生系统性偏移，使F1分数显著下降。

\textbf{数据标签识别错误。}部分样本中，模型正确识别了信号名称与波形结构，但数据总线的数值标签存在少量错误。例如在7个信号的样本中，仅有1至2个数据标签与标准答案不一致。这类错误反映了时序图中小字号十六进制数值的视觉识别难度，尤其当多位数据标签紧密排列时，字符间的区分度降低。相比波形长度偏差，数据标签错误的影响范围局限于个别信号的个别时钟周期，对整体F1分数的影响较小。
...
整体而言，第一阶段模型在时序图到WaveJSON的转换任务上表现优异：信号名称的识别几乎完全正确（所有成功解析的样本均正确识别了全部信号名），波形结构的还原精度高，主要瓶颈在于长时序图的周期计数精度与密集数据标签的精确识别。
...
为评估微调的效果，本文将微调模型与多个通用大语言模型进行对比，使用相同的测试集与评估流程。基线模型均以零样本方式直接推理。表~\ref{tab:stage2-comparison}汇总了各模型的对比结果。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcrr}
    ...
    模型 & 参数量 & 通过仿真 & 平均F1 \\
    \midrule
    GLM-4.5-Air\inlinecite{glm2025chatglm} & 106B (A12B) & 35/117 & 0.253 \\
    ...
    GPT-OSS-120B\inlinecite{openai2025gptoss} & 120B & 67/117 & 0.473 \\
    \midrule
...
通用模型在零样本设置下的整体F1均低于微调模型。GPT-OSS-120B在基线中表现最优（F1=0.473），其仿真通过率（67/117=57.3\%）也是基线中最高的，但仍显著低于本文8B参数微调模型的67.5\%。微调模型在仿真通过率与整体F1上均取得了最优结果，体现了领域特定训练对代码结构规范性与功能正确性的提升。
...
这一错误模式具有重要的方法论启示。有监督微调的词元级损失函数对功能正确性的监督是间接的——它优化的是生成序列与参考代码之间的词元匹配度，而非生成代码的功能行为。上述案例表明，词元级匹配能够引导模型学会代码的结构模式，但不足以保证行为层面的正确性。同时，这类错误产生的部分正确输出——F1分布在0.25至0.58之间，而非全零——恰好为基于仿真奖励的强化学习提供了有意义的优化梯度。第~\ref{chap:method}~章提出的GRPO方法正是针对这一瓶颈：以仿真对拍的F1分数作为连续值奖励信号，直接在功能正确性层面优化模型，使其超越有监督微调所能达到的性能上限。
