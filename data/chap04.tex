...
\textbf{状态机对比。}%
图~\ref{fig:fsm-comparison}展示了一个多状态FSM模块在两种激励下的时序图。该模块以两个输入信号\texttt{a}、\texttt{b}驱动状态转移，通过3位输出\texttt{dout}指示当前状态。随机激励方案中，gentbvlog正确识别了时钟信号并生成了周期性驱动，但将其余三个输入端口（复位与两路输入）打包为一个3位数组，统一由\texttt{\$random}驱动。随机的复位信号使模块长时间处于复位状态，释放后输入序列也缺乏功能语义，导致状态机几乎始终停留在同一状态，输出\texttt{dout}在整个仿真过程中仅出现极少数值变化。LLM激励方案在分析模块的状态转移逻辑后，设计了覆盖性的输入序列：先完成规范的复位初始化，随后系统地组合\texttt{a}与\texttt{b}的不同取值，驱动状态机遍历各个状态转移路径。输出\texttt{dout}在20个时钟周期内循环经过全部状态值，完整展示了状态机的转移行为。

\begin{figure}[htbp]
  \centering
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/samples/fsm.png}
    \centerline{（a）随机激励}
  \end{minipage}
  \vspace{0.5em}
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=\textwidth]{figures/samples/fsm_llm.png}
    \centerline{（b）LLM激励}
  \end{minipage}
...
图~\ref{fig:sample-diversity}展示了数据集中不同功能类别模块的时序图样本，体现了数据集在信号类型与波形模式上的多样性。全加器模块以组合逻辑为主，输入与输出均为多位数据总线，波形呈现密集的数据标签变化；UART控制器模块展示了串行通信接口的握手信号交互模式；RAM模块则呈现了存储器读写操作的典型地址-数据时序关系。

\begin{figure}[htbp]
  \centering
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.7\textwidth]{figures/samples/adder.png}
    \centerline{（a）全加器（\texttt{Calculator\_Full\_Adder}）}
  \end{minipage}
  \vspace{0.5em}
  \begin{minipage}{\textwidth}
    \centering
    \includegraphics[width=0.7\textwidth]{figures/samples/uart.png}
